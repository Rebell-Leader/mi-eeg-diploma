...
def train_model(dataloaders, model, criterion, optimizer, device,
    ...
    for epoch in range(epochs):
        model, train_loss = train_epoch(dataloaders['train'], model, criterion, optimizer, device,
        ...
        if verbose:
            message = "Epoch {:d}/{:d} => Loss {:4f} => Train Acc {:2f} => Val Acc {:2f} => Test Acc {:2f}".format(
                epoch + 1, epochs, train_loss, train_acc, val_acc, test_acc)
            print(message)
