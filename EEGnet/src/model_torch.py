...
class deepwise_separable_conv(nn.Module):
    ...
    def get_output_size(self, h_w):
        return convtransp_output_shape(h_w, kernel_size=(1, self.kernelSize), stride=1, pad=(0, self.time_padding),
                                       dilation=1)
...
class EEGNet_experimental(nn.Module):
    ...
    def __init__(self, nb_classes, Chans=64, Samples=128,
        ...
        self.output_sizes['avg_pool_block1'] = convtransp_output_shape(self.output_sizes['depthwise1'],
                                                                       kernel_size=(1, poolKern1),
...
if __name__ == '__main__':
    ...
    all_subjects = [1]
    eeg_ch = range(21)
    subjects = data.get_data(all_subjects, eeg_ch, shuffle=True, windows=(-0.3, 0), baseline_window=(-0.1, 0),
                             resample_to=500)
    # X_train, Y_train, X_test, Y_test = train_test_split(subjects, 8, 2)
    ...
    #           'lr': 0.00066,
    #           }
    model = EEGNet_experimental(nb_classes=2, Chans=21, Samples=151,
                                dropoutRates=(0.25, 0.25), kernLength1=64, kernLength2=16, poolKern1=4, poolKern2=8,
                                F1=4,
                                D=2, F2=8, norm_rate=0.25, dropoutType='Dropout')
    output = model.forward(torch.Tensor(subjects[1][0][:, np.newaxis, ...]))
    # new_model = train_model_eegnet(X_train, Y_train, params, (X_test, Y_test), epochs=1, batch_size=64, shuffle=True,
    #                                model_path=model)
    pass